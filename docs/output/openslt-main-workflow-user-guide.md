...
创建运行只生成任务和配置快照，不会立即执行远端命令。进入运行详情后还需单击“启动运行”。运行启动后，系统先排队获取资源锁，再自动执行配置采集。
...
1. 当前任务区显示准备环境、执行测速、分析与复核三个阶段，以及当前需要处理的操作。
2. 展开“执行详情”查看完整节点、耗时、Trace ID 和日志；可选择历史节点查看记录，再返回当前任务。
3. 统计结果直接展示，分析历史和详细执行记录按需展开；“结果与报告”集中呈现指标、历史对比、复核和报告下载。

新建运行的基本操作顺序：

1. 单击“启动运行”。
2. 等待资源排队和连续配置采集完成；采集失败会停在当前节点，处理后重试。
3. 接线节点直接展示待确认拓扑。运行页自动连接并启动 REM、SLNIC、模拟市场和解析终端；连接失败可手动重新连接，发单工具仍需先确认本次配置。
4. 根据提示处理现场操作，单击“确认接线并继续”或“确认完成并继续”，平台衔接下一节点。
5. 统计节点选择 CSV 和上限，点击“保存并分析”；采用结果后进入“结果与报告”复核，点击“确认结论并生成报告”。
...
- 系统启动发单会话后，可直接在 SSH 终端输入命令，也可使用下方动作按钮发送常用指令。
- 每次单击动作按钮后检查“最近发送”状态。
...
- 操作员只能选择当前节点之前最近一次成功解析生成的 CSV，并须同时填写正整数“异常大值上限”。单击“保存并分析”会在同一事务中保存 CSV、上限并预留本次分析编号；提交失败不会启动分析。
- 在节点完成前，可按需要修改配置后反复分析。每次开始或重试都会预留一条按分析编号排序的历史记录；分析完成后该记录会标记为成功或失败，并保留当时的 CSV、上限、脚本、结果或错误。
...
1. 统计结果采用后进入待复核状态，页面自动切换到“结果与报告”。
...
在“结果与报告”页签展开“与历史运行对比”。当前运行产生统计指标后，系统会列出同场景已完成运行，并优先推荐工作流版本、统计脚本、异常大值上限和指标单位一致且人工结论为“通过”的运行。

1. 选择基线运行；先检查匹配指标数量和可比性说明。
2. 如果工作流、脚本、异常大值上限或单位不一致，系统会标记为不可完全比较；资源集合不同也会明确提示。确认差异不会影响结论后，才可继续保存。
3. 单击“保存对比快照”。系统会固定双方当前采用的统计分析批次，不会因后续重复分析而静默改变历史结果。
4. 检查基线值、当前值、绝对变化和变化率。当前统计指标均为延迟指标，因此下降表示改善、上升表示退化；新增、缺失和不可比指标不会被误判为改善或退化。
5. 如果当前运行或基线运行在保存后重新分析，页面会提示快照状态。需要采用新结果时，重新保存对比快照。

在待复核阶段保存对比快照后，提交结论时会统一生成报告；运行已完成时更新对比快照，应在“全部产物与历史报告”重新生成报告。新版本 HTML、Excel 和 PDF 会包含该快照，已有历史报告保持不变。
...
3. 在同页单击“确认结论并生成报告”，保存结论并生成报告后运行才标为完成。
...
在“结果与报告”下载最新报告；完整中间产物和历史版本位于“全部产物与历史报告”。

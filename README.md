...
管理员可在 Web 端“资源管理”中新建 MySQL 数据库资源，选择直接连接或 SSH 隧道，并为同一服务器配置多个数据库名称。管理员和测试人员可从资源列表进入数据库操作台，执行 SELECT、导出 CSV/XLSX，或在两次确认后执行单表 UPDATE。

`EXECUTION_MODE=simulated` 时数据库操作返回带明确标记的模拟结果；切换为 `remote` 并重启后才会访问真实 MySQL。UPDATE 必须包含 WHERE，不支持 JOIN、子查询、ORDER、LIMIT 或多语句，且单次最多影响 1000 行。

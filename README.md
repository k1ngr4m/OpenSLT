...
每库可同时使用 SVN 与手动上传；SVN 保留多仓库、路径白名单和每 30 分钟自动同步，上传文件单个最大 50 MiB。
...
默认每位用户同时生成 1 个回答，全平台最多 4 个；总生成超时 180 秒，最大输出 2048 tokens，
上下文最多 24000 字符且仅携带近期完整问答。管理员可通过 `.env.example` 中的 `CHAT_*` 设置
调整限制。`CHAT_MIN_VECTOR_SCORE` 默认 0.35，应结合实际 Embedding 模型与业务问题校准，

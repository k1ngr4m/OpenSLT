...
管理员和测试人员可从首页导航进入“智能助手”，使用通用对话或基于所选知识库的知识问答。
...
Embedding 模型由系统管理员保存，每个知识库独立绑定一个模型及其索引。新建知识会话时选择知识库；历史会话保持原库，换库请新建会话。智能用例也先选知识库，再选择需求。

可在智能助手顶部切换已保存的对话模型，与当前账户的模型管理同步，从下一条消息生效；历史回答保留实际使用的模型名称。
支持流式回答、多轮追问、个人历史、删除对话、停止生成和失败后重新提问。知识回答可展开
查看参考资料路径、revision、片段编号及当时发送给模型的脱敏内容；相关性不足时明确提示未找到资料。
来源片段用于人工核对，不能保证模型的每个结论都正确。通用对话不检索项目资料。

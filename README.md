...
- 固定同场景历史运行的统计分析批次，对比基线与当前指标并将结果纳入报告。
...
管理中心的“知识库”支持创建多个知识库，填写名称、描述并选择已保存的 Embedding 模型。
进入知识库后，可查看概览、管理文档、检索资料和调整设置。管理员管理配置与文档，测试人员可查看和检索。
每库可同时使用 SVN 与手动上传；SVN 保留多仓库、路径白名单和每 30 分钟自动同步，DOCX、XLSX 上传文件单个最大 500 MiB，其他格式最大 50 MiB，提取文本最多 1000 万字符。
文件和索引按知识库隔离；同库索引任务串行，失败或取消保留上次成功索引，删除文件在新索引发布后完成。
Embedding 绑定后固定；分块默认 1200 字符、重叠 150 字符，修改后需重建索引；检索数量默认 10。

升级执行 `python -m alembic upgrade head` 后，已有知识源、知识会话和用例记录归入“默认知识库”。
服务启动时复制匹配的旧索引与 SVN 工作副本到 `KNOWLEDGE_ROOT/<知识库 ID>/`，保留旧文件；备份时应保留整个 `KNOWLEDGE_ROOT`。
缺少模型或索引不匹配时会提示补选模型或重建索引。含多个库或上传文档时，迁移拒绝直接降级为单知识源。

## 智能助手

管理员和测试人员可从首页导航进入“智能助手”，直接提问，或通过输入区最左侧“+”上传附件、选择知识库。
对话和智能用例统一使用“模型管理 → 对话”中当前账户启用的模型，各账户配置与 API Key 相互隔离，未配置时不会回退到其他账户的模型。
升级时执行 `alembic upgrade head`：已有个人 LLM 配置迁入对应账户，原共享对话模型归最早创建的系统管理员；个人配置优先作为当前模型。
Embedding 模型由系统管理员保存，每个知识库独立绑定一个模型及其索引。知识库为可选上下文，可在同一对话中切换或移除，从下一条消息生效。智能用例也先选知识库，再选择需求。

可在发送按钮左侧切换已保存的对话模型，与当前账户的模型管理同步，从下一条消息生效；历史回答保留实际使用的模型名称。
支持流式回答、多轮追问、个人历史、删除对话、停止生成和失败后重新提问。知识回答可展开
查看参考资料路径、revision、片段编号及当时发送给模型的脱敏内容；相关性不足时明确提示未找到资料。
来源片段用于人工核对，不能保证模型的每个结论都正确。未选择知识库时不检索项目资料。

附件支持 PDF、Word、Excel 及文本格式，每条消息最多 3 个。DOCX/XLSX 最大 500 MiB，其他格式最大 50 MiB，提取文本上限 1000 万字符；每个附件保留前 12000 字符，并明确展示节选范围。解析正文随私人消息保存，用于当前问题及上下文容纳的后续追问，原文件解析后清除。扫描件需先做 OCR。升级需执行 `alembic upgrade head` 添加附件字段。

聊天仅提供文字回答，不执行 SSH、SQL 或测试操作。每个用户仅可读取或删除自己的会话，
对话正文和引用不进入共享 HTTP 日志；审计记录操作、消息 ID 和模型名称。历史消息保存在
应用数据库中并随其备份，点击删除对话会删除该会话及消息。停止或连接中断时保存已收到的
部分回答，未完成回答不会加入后续上下文；服务重启后将未完成消息标记为已中断。

默认每位用户同时生成 1 个回答，全平台最多 4 个；总生成超时默认 300 秒，最大输出 2048 tokens，
上下文最多 24000 字符且仅携带近期完整问答。管理员可通过 `.env.example` 中的 `CHAT_*` 设置
调整限制；已有部署若显式配置了 `CHAT_TIMEOUT_SECONDS=180`，请改为 `300` 并重启后端。`CHAT_MIN_VECTOR_SCORE` 默认 0.35，应结合实际 Embedding 模型与业务问题校准，
它是检索过滤阈值，不是回答可信度。索引仍使用现有 SQLite 检索，无需额外向量数据库。

升级时安装更新后的运行依赖（包含 HTTPX），执行 `python -m alembic upgrade head`，重新构建
前端并更新 Nginx 配置。聊天采用单个 API worker 内的并发与取消管理，与当前部署方式一致；
不要直接扩展为多 worker。聊天模型请求不依赖环境代理，也不跟随重定向。

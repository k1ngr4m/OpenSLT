...
- `VERSION` 是 OpenSLT 应用版本的唯一来源。请使用不带 `v` 前缀、格式为
  `MAJOR.MINOR.PATCH` 的稳定语义化版本。不要在 Python、前端或部署脚本中硬编码另一套
  应用版本。
- 将用户可见以及与运维相关的变更记录在 `RELEASES.json` 的 `unreleased` 数组中。
  纯测试改动、格式调整和内部重构无需添加发布说明，除非它们会影响部署或运维人员。
- 每条变更记录使用 `added`、`changed`、`fixed`、`removed` 或 `security` 类型。说明文字
  应简短并面向用户。
- 发布版本时，应根据兼容性选择语义化版本号：不兼容的变更递增 `MAJOR`，向后兼容的
  功能递增 `MINOR`，兼容性修复递增 `PATCH`。将所有未发布条目移入 `releases` 开头的
  新发布记录中，添加 `YYYY-MM-DD` 格式的日期和标题，清空 `unreleased`，并在同一次
  变更中更新 `VERSION`。
- 编辑版本元数据后，运行 `python tools/release_metadata.py`。如果最新发布版本与
  `VERSION` 不一致、版本重复或顺序错误，或者记录不完整，则该发布无效。
- 制作离线包之前，运行后端测试、前端测试和前端生产构建。wheel 版本、前端版本、
  软件包文件名、软件包内的 `VERSION` 以及已安装软件包的版本都必须与仓库中的
  `VERSION` 一致。

...
it('selects all or individual cases and fields, retains input on failure and resets on another preview', async () => {
  ...
  try {
    ...
    const allCases = wrapper.get<HTMLInputElement>('input[aria-label="全选用例"]')
    const allFields = wrapper.get<HTMLInputElement>('input[aria-label="全选字段"]')
    expect(allCases.element.checked).toBe(false)
    expect(allCases.element.indeterminate).toBe(false)
    expect(allFields.element.indeterminate).toBe(true)
    await allCases.setValue(true)
    expect(wrapper.get('.revise-button').text()).toContain('已选 2 条')
    await wrapper.get('input[aria-label="选择用例 TC-0001"]').setValue(false)
    expect(allCases.element.checked).toBe(false)
    expect(allCases.element.indeterminate).toBe(true)
    await allCases.setValue(true)
    expect(allCases.element.indeterminate).toBe(false)
    await allCases.setValue(false)
    expect(wrapper.get('.revise-button').text()).toContain('已选 0 条')
    await allFields.setValue(true)
    expect(allFields.element.indeterminate).toBe(false)
    await allFields.setValue(false)
    expect(wrapper.findAll<HTMLInputElement>('fieldset input').every(input => !input.element.checked)).toBe(true)
    await wrapper.get('fieldset input[value="title"]').setValue(true)
    expect(allFields.element.indeterminate).toBe(true)
    ...
    await wrapper.get('.case-revision').trigger('submit')
    await flushPromises()
    expect(post).toHaveBeenLastCalledWith('/smart-cases/generations/1/revise', { case_indices: [0, 1], fields: ['title', 'preconditions', 'steps', 'expected_results', 'case_type', 'priority'], instruction: '名称更清晰' })

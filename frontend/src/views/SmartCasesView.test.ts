...
describe('SmartCasesView SVN knowledge source', () => {
  it('keeps credentials write-only and exposes the required safety controls', () => {
    ...
    expect(source).toContain('测试 SVN 连接')
    expect(source).toContain('{ timeout: 0 }')
    expect(source).not.toContain('OpenAI-compatible Base URL')
    expect(source).not.toContain('Embedding Base URL')

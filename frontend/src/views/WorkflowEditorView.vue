    ...
    <el-tabs v-model="activeTab" class="workflow-tabs" :before-leave="beforeTabLeave">
      ...
      <el-tab-pane label="流程编辑" name="workflow">
        ...
        <template v-if="selectedNode">
          ...
          <label class="ui-field-row field"><span>节点名称</span><el-input v-model="selectedNode.name" :disabled="!editable" maxlength="128" @input="markDirty" /></label>
          ...
          <template v-else-if="selectedNode.node_type === 'database_config'">
            <label class="ui-field-row field"><span>配置数据库</span><el-select v-model="selectedNode.config.database_name" :disabled="!editable" filterable @change="selectDatabaseConfigDatabase"><el-option v-for="name in selectedResourceMap.database?.database_names || []" :key="name" :label="name" :value="name" /></el-select></label>
          ...
          <template v-else-if="selectedNode.node_type === 'order_preparation'">
            <label class="ui-field-row field required"><span>XML 配置</span><el-select v-model="selectedNode.config.xml_filename" :disabled="!editable || !selectedResourceMap.order" filterable @change="selectXml"><el-option v-for="file in orderConfigs" :key="file.name" :label="file.name" :value="file.name" /></el-select></label>
            <label class="ui-field-row field"><span>网卡接口</span><el-input v-model="selectedNode.config.network_interface" :disabled="!editable" placeholder="例如 p4p1" maxlength="15" @input="markDirty" /><small>运行时安全生成 ZF_ATTR。</small></label>
            ...
            <template v-if="selectedNode.config.read_symbol_csv">
              <label class="ui-field-row field"><span>交易数据库</span><el-select v-model="selectedNode.config.trading_database_name" :disabled="!editable" filterable @change="markDirty"><el-option v-for="name in selectedResourceMap.database?.database_names || []" :key="name" :label="name" :value="name" /></el-select><small>从数据库资源白名单中选择对应的 *_trading_data 库。</small></label>
          ...
          <template v-else-if="selectedNode.node_type === 'rem_startup'">
            ...
            <label class="ui-field-row field"><span>执行命令（按顺序）</span><el-input
          ...
          <template v-else-if="slnicNodeTypes.has(selectedNode.node_type)">
            ...
            <template v-if="selectedNode.node_type === 'slnic_merge_capture'">
              <label class="ui-field-row field required">
          ...
          <template v-else-if="selectedNode.node_type === 'parser_parse'">
            <label class="ui-field-row field required"><span>运行数据库</span><el-select v-model="selectedNode.config.database_name" :disabled="!editable || !selectedResourceMap.database" filterable @change="markDirty"><el-option v-for="name in selectedResourceMap.database?.database_names || []" :key="name" :label="name" :value="name" /></el-select><small>三张订单表从该 *_trading_data 库导出；t_account_exchange_code 从同前缀 *_config 库导出。</small></label>
          ...
          <template v-else-if="selectedNode.node_type === 'data_statistics'">
            ...
            <label v-if="!selectedNode.config.engine || selectedNode.config.engine === 'remote'" class="ui-field-row field required">
              <span>远端统计脚本</span>
...
<style scoped>
.property-panel { --ui-field-label-width: 104px; }
.resource-fields > .ui-field-row, .property-panel .ui-field-row {
  display: grid; grid-template-columns: var(--ui-field-label-width) minmax(0,1fr); align-items: start; gap: 6px 12px;
}
.property-panel .ui-field-row > span:first-child, .resource-fields > .ui-field-row > span:first-child { margin: 0; padding-top: 7px; line-height: 1.5; }
.property-panel .ui-field-row > small { grid-column: 2; margin: 0; }

.workflow-page{min-height:calc(100vh - 58px);background:#edf2f4;color:#1d2c34}.workflow-header{height:78px;padding:0 20px;background:var(--ui-surface);border-bottom:1px solid #dce4e8;display:flex;align-items:center;justify-content:space-between}.header-left,.title-line,.header-actions,.property-title,.contract-toolbar{display:flex;align-items:center}.header-left{gap:10px}.header-left h1{font-size:18px;margin:0}.title-line{gap:9px}.header-left p{margin:5px 0 0;color:var(--ui-text-secondary);font-size:12px}.dirty-mark{font-size:12px;color:#b7791f}.header-actions{gap:9px}.editor-grid{display:grid;grid-template-columns:240px minmax(430px,1fr) 390px;height:calc(100vh - 136px);min-height:660px}.resource-panel,.property-panel{background:var(--ui-surface);overflow:auto}.resource-panel{border-right:1px solid #dce4e8;padding:18px}.property-panel{border-left:1px solid #dce4e8;padding:20px}.panel-heading strong,.panel-heading small,.property-title strong,.property-title small{display:block}.panel-heading small,.property-title small{color:var(--ui-text-secondary);font-size:12px;margin-top:4px}.resource-fields{display:grid;gap:13px;margin-top:20px}.resource-fields label,.field{display:grid;gap:6px}.resource-fields label>span,.field>span{font-size:12px;font-weight:600;color:#52616a}.resource-note{margin-top:22px;padding:12px;border-radius:8px;background:#edf7f4;color:#37675b}.resource-note strong,.resource-note span{display:block}.resource-note span{font-size:12px;line-height:1.6;margin-top:5px}.workflow-canvas{overflow:auto;padding:18px 28px 60px;background-color:#f4f7f8;background-image:radial-gradient(#d7e0e4 1px,transparent 1px);background-size:22px 22px}.canvas-intro{display:flex;align-items:baseline;justify-content:space-between;color:#71808a;font-size:12px}.canvas-intro strong{font-size:14px;color:#34444d}.flow-column{width:360px;margin:22px auto;display:flex;flex-direction:column;align-items:center}.add-point{width:30px;height:30px;border:1px solid #cbd7dc;border-radius:7px;background:var(--ui-surface);color:#268b77;display:grid;place-items:center;cursor:pointer;box-shadow:0 3px 10px rgba(34,61,72,.08)}.flow-node{width:360px;min-height:108px;padding:15px;background:var(--ui-surface);border:1px solid #d9e2e6;border-left:4px solid #48a895;border-radius:10px;display:grid;grid-template-columns:42px minmax(0,1fr) auto;gap:12px;align-items:start;box-shadow:0 8px 24px rgba(35,58,68,.08);cursor:pointer}.flow-node.selected{border-color:#22a68e;box-shadow:0 0 0 3px rgba(34,166,142,.13),0 10px 28px rgba(35,58,68,.1)}.flow-node.blue{border-left-color:#4f8fbd}.flow-node.amber{border-left-color:#d29b42}.flow-node.rose{border-left-color:#bd6b78}.node-icon{width:36px;height:36px;border-radius:8px;background:#e1f3ee;color:#248b76;display:grid;place-items:center;flex:0 0 auto}.node-icon.blue{background:#e7f0f7;color:#3f7ca7}.node-icon.amber{background:#f8eedc;color:#a77525}.node-icon.rose{background:#f7e7ea;color:#a65361}.node-copy>span,.node-copy>strong,.node-copy>small{display:block}.node-copy>span{font-size:12px;color:#8a98a0}.node-copy>strong{font-size:14px;margin-top:5px}.node-copy>small{font-size:12px;color:#71808a;margin-top:8px}.node-actions{display:flex;flex-direction:column;opacity:.25}.flow-node:hover .node-actions,.flow-node.selected .node-actions{opacity:1}.node-actions :deep(.el-button){margin:0}.flow-link{position:relative;width:30px;height:54px;display:grid;place-items:center}.flow-link>span{position:absolute;top:0;bottom:8px;left:50%;border-left:2px solid #ccd7dc;transform:translateX(-1px)}.flow-link>span::after{position:absolute;bottom:-8px;left:50%;width:0;height:0;border-top:8px solid #ccd7dc;border-right:6px solid transparent;border-left:6px solid transparent;content:'';transform:translateX(-50%)}.flow-link .add-point{position:relative;z-index:1;width:28px;height:28px}.flow-empty,.property-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;color:#87949c}.flow-empty{width:360px;height:180px;margin-top:14px;border:1px dashed #bccbd1;border-radius:12px;background:rgba(255,255,255,.7)}.flow-empty :deep(svg),.property-empty :deep(svg){width:30px;height:30px}.flow-empty strong,.property-empty strong{color:#52616a;margin-top:12px}.flow-empty span,.property-empty span{font-size:12px;margin-top:5px}.flow-end{color:#87949c;font-size:12px;display:flex;flex-direction:column;align-items:center}.flow-end span{height:18px}.property-title{gap:10px;padding-bottom:16px;border-bottom:1px solid #e4eaed}.property-panel .field{margin-top:17px}.field small{color:#829099;font-size:12px;line-height:1.5}.required>span:after{content:' *';color:#d04b5d}.section-label{font-size:12px;font-weight:700;color:#46565f;margin:20px 0 10px}.target-box{padding:12px;margin-bottom:10px;border:1px solid #e0e7ea;border-radius:8px}.target-box.disabled{background:#f6f8f9}.target-box :deep(.el-checkbox__label) strong,.target-box :deep(.el-checkbox__label) small{display:block}.target-box :deep(.el-checkbox__label) small{font-size:12px;color:#8a979f}.target-box :deep(.el-checkbox-group){display:grid;grid-template-columns:1fr 1fr;margin:10px 0 0 24px}.key-toolbar{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center;margin-bottom:8px}.key-toolbar :deep(.el-button){min-width:64px;margin:0}.key-grid{max-height:310px;overflow:auto;padding:10px;border:1px solid #e0e7ea;border-radius:8px}.key-options{display:grid;grid-template-columns:1fr}.key-options :deep(.el-checkbox){margin-right:0}.key-options :deep(.el-checkbox__label){font:12px/1.4 Cascadia Code,Consolas,monospace}.key-grid-empty{min-height:72px;display:grid;place-items:center;color:#87949c;font-size:12px}.wiring-placeholder{margin-top:20px;min-height:260px;border:1px dashed #d6b56e;border-radius:10px;background:repeating-linear-gradient(45deg,#fffaf0,#fffaf0 10px,#fdf6e8 10px,#fdf6e8 20px);display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;color:#9a762d;padding:24px}.wiring-placeholder :deep(svg){width:44px;height:44px}.wiring-placeholder strong{margin-top:14px}.wiring-placeholder span{font-size:12px;line-height:1.6;margin-top:7px;max-width:240px}.contract-toolbar{justify-content:space-between;margin-top:18px}.contract-list{display:grid;margin-top:10px;border:1px solid #e0e7ea;border-radius:8px;max-height:200px;overflow:auto}.contract-list :deep(.el-checkbox){height:auto;margin:0;padding:10px;border-bottom:1px solid #edf1f3}.contract-list :deep(.el-checkbox:last-child){border-bottom:0}.contract-list strong,.contract-list small{display:block}.contract-list small{font-size:12px;color:var(--ui-text-secondary);margin-top:3px}.preview-results{margin-top:22px}.snapshot{border:1px solid #e0e7ea;border-radius:8px;margin-bottom:10px;padding:11px}.snapshot>div{display:flex;justify-content:space-between;align-items:center}.snapshot dl{display:grid;grid-template-columns:120px 1fr;margin:10px 0 0;font-size:12px}.snapshot dt,.snapshot dd{padding:5px 0;border-top:1px solid #edf1f3}.snapshot dt{color:#77858e}.snapshot dd{margin:0;word-break:break-word}.snapshot dd.failed{color:#c74d5d}.property-empty{height:100%}.node-picker{display:grid;gap:10px}.node-picker button{width:100%;display:flex;align-items:center;gap:12px;padding:14px;border:1px solid #dfe6ea;border-radius:9px;background:var(--ui-surface);text-align:left;cursor:pointer}.node-picker button:hover{border-color:#55aa98;background:#f2f9f7}.node-picker strong,.node-picker small{display:block}.node-picker small{margin-top:5px;color:#7c8a93;font-size:12px}.teal{--node-tone:#48a895}@media(max-width:1250px){.editor-grid{grid-template-columns:210px minmax(410px,1fr) 340px}.flow-node,.flow-empty{width:330px}.flow-column{width:330px}}
.node-picker button:disabled{cursor:not-allowed;opacity:.55;background:#f5f7f8}
.flow-node,.flow-empty,.wiring-placeholder,.node-picker button{border-radius:8px}.wiring-editor-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin:20px 0 10px}.wiring-editor-heading strong,.wiring-editor-heading span{display:block}.wiring-editor-heading span{max-width:240px;margin-top:4px;color:var(--ui-text-tertiary);font-size:12px;line-height:1.45}.wiring-editor-heading+.el-alert{margin-bottom:10px}.contract-previews{margin-top:12px}.contract-preview-title{display:flex;align-items:center;gap:8px;min-width:0}.contract-preview-title small{color:var(--ui-text-secondary)}.checksum{display:grid;gap:4px;margin-bottom:10px}.checksum span{font-size:12px;color:var(--ui-text-secondary)}.checksum code{font-size:12px;line-height:1.5;overflow-wrap:anywhere;color:#34444d}.snapshot dl{grid-template-columns:minmax(0,1fr) minmax(96px,35%);column-gap:12px}.snapshot dt,.snapshot dd{min-width:0;line-height:1.45;overflow-wrap:anywhere}.snapshot dt{font-family:Cascadia Code,Consolas,monospace;font-size:12px}.snapshot dd{font-variant-numeric:tabular-nums}@media(max-width:1250px){.editor-grid{grid-template-columns:180px minmax(320px,1fr) 300px}.resource-panel{padding:12px}.property-panel{padding:14px}.workflow-canvas{padding:16px 12px 48px}.flow-column,.flow-node,.flow-empty{width:290px}}
.contract-empty{min-height:96px;margin-top:10px;border:1px dashed #ccd8dd;border-radius:8px;display:grid;place-items:center;color:#829099;font-size:12px}
.flow-node.violet{border-left-color:#7669b5}.node-icon.violet{background:#eeebf8;color:#6556a5}.node-catalog{display:grid;gap:22px}.node-category h3{margin:0 0 9px;padding-bottom:7px;border-bottom:1px solid #e6ebee;color:var(--ui-text-secondary);font-size:12px;font-weight:600}.slnic-summary{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:18px 0 14px}.slnic-summary>div{padding:11px;border:1px solid var(--ui-border);border-radius:8px;background:var(--ui-surface-subtle)}.slnic-summary .wide{grid-column:1/-1}.slnic-summary span,.slnic-summary strong,.slnic-summary code{display:block}.slnic-summary span{font-size:12px;color:var(--ui-text-secondary)}.slnic-summary strong,.slnic-summary code{margin-top:5px;font-size:12px;overflow-wrap:anywhere}.slnic-commands{display:grid;gap:7px}.slnic-commands code{padding:10px;border-radius:7px;background:#242632;color:#d9e6df;font-size:12px;line-height:1.45;overflow-wrap:anywhere}.slnic-note{color:var(--ui-text-secondary);font-size:12px;line-height:1.6}
...
.windows-command-preview{display:block;padding:11px;border-radius:7px;background:#242632;color:#d9e6df;font:12px/1.6 "Cascadia Code","JetBrains Mono",Consolas,monospace;overflow-wrap:anywhere}
.market-script-options{display:grid;max-height:190px;margin-top:10px;overflow:auto;border:1px solid #e0e7ea;border-radius:8px}.market-script-options :deep(.el-checkbox){height:auto;margin:0;padding:9px 11px;border-bottom:1px solid #edf1f3}.market-script-options :deep(.el-checkbox:last-of-type){border-bottom:0}.market-script-options strong,.market-script-options small{display:block}.market-script-options small{margin-top:3px;color:var(--ui-text-secondary);font-size:12px}.market-script-order{display:grid;gap:7px}.market-script-row{display:grid;grid-template-columns:24px minmax(0,1fr) auto;gap:8px;align-items:center;padding:9px;border:1px solid #dfe6ea;border-radius:7px}.market-script-row.invalid{border-color:#e3b268;background:#fffaf1}.market-script-row strong,.market-script-row small{display:block;overflow-wrap:anywhere}.market-script-row small{margin-top:3px;color:var(--ui-text-secondary);font-size:12px}.market-script-row.invalid small{color:#a66e20}.market-script-index{display:grid;width:22px;height:22px;place-items:center;border-radius:5px;background:#edf3f4;color:var(--ui-text-secondary);font-size:12px;font-weight:700}.market-script-actions{display:flex;align-items:center}.market-script-actions :deep(.el-button){margin:0}

.template-toolbar{display:grid;grid-template-columns:minmax(0,1fr) repeat(3,32px);gap:6px;align-items:center}.template-toolbar :deep(.el-button){width:32px;height:32px;margin:0}.catalog-alert{margin-bottom:8px}.catalog-alert :deep(.el-alert__content){min-width:0}.stale-key-row{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px;padding:8px 10px;border:1px solid #efd39a;border-radius:6px;background:#fff9ec;color:#8a641f;font-size:12px}.stale-key-row :deep(.el-button){flex:0 0 auto}.key-grid{min-height:94px}.key-options :deep(.el-checkbox){height:auto;min-height:36px;padding:5px 0;align-items:flex-start}.key-options :deep(.el-checkbox__input){margin-top:2px}.key-options :deep(.el-checkbox__label){min-width:0;white-space:normal;font-family:inherit}.key-option-copy,.key-option-copy strong,.key-option-copy small{display:block}.key-option-copy strong{font:12px/1.4 Cascadia Code,Consolas,monospace;overflow-wrap:anywhere}.key-option-copy small{margin-top:2px;color:var(--ui-text-secondary);font-size:12px;line-height:1.45}

/* Professional console theme and compact-window fallback */
.workflow-page{min-height:calc(100dvh - 52px);background:var(--ui-surface-subtle);color:var(--ui-text-primary)}
.workflow-header{border-color:var(--ui-border)}
...
.workflow-canvas{background-color:var(--ui-surface-subtle);background-image:radial-gradient(#d1dddf 1px,transparent 1px)}
...
.flow-node.selected{border-color:var(--ui-primary);box-shadow:0 0 0 3px rgba(53, 105, 184,.12),0 10px 24px rgba(35,58,68,.09)}

...
import { ArrowLeft, Bottom, Check, Connection, Delete, Document, Plus, Promotion, Refresh, Tickets, Top } from '@element-plus/icons-vue'
      ...
      <aside class="property-panel">
        <template v-if="selectedNode">
          ...
          <template v-else-if="selectedNode.node_type === 'database_config'">
            ...
            <el-checkbox-group v-model="selectedNode.config.keys" class="key-grid" :disabled="!editable" @change="markDirty"><el-checkbox v-for="key in GLOBAL_KEYS" :key="key" :label="key">{{ key }}</el-checkbox></el-checkbox-group>
            <el-button :icon="Refresh" :loading="previewing" :disabled="!editable" @click="previewNode">预采集并保存</el-button>
          </template>

...
.flow-node.selected{border-color:var(--ui-primary);box-shadow:0 0 0 3px rgba(0,168,143,.12),0 10px 24px rgba(35,58,68,.09)}
...
.version-trigger{display:flex;align-items:center;gap:7px;margin:5px 0 0 -6px;padding:3px 5px 3px 6px;border:0;border-radius:8px;color:var(--ui-text-secondary);background:transparent;font-size:12px;cursor:pointer;transition:color var(--ui-transition),background-color var(--ui-transition),transform var(--ui-transition)}.version-trigger:hover{color:var(--ui-primary);background:rgba(0,168,143,.07)}.version-trigger:active{transform:translateY(1px)}.version-trigger:focus-visible{outline:2px solid var(--ui-primary);outline-offset:2px}.version-trigger-icon{display:grid;width:24px;height:24px;place-items:center;border:1px solid transparent;border-radius:7px;color:var(--ui-primary);background:rgba(0,168,143,.08);transition:background-color var(--ui-transition),border-color var(--ui-transition),color var(--ui-transition)}.version-trigger-icon :deep(svg){width:14px;height:14px}.version-trigger:hover .version-trigger-icon{border-color:rgba(0,168,143,.22);background:#fff;color:var(--ui-primary-hover)}
...
/* Visual Design 2.0 */
.workflow-page{background:var(--ui-canvas)}
.workflow-header{border-color:var(--ui-border);background:var(--ui-surface)}
.header-left h1{font-size:20px}.header-left p{font-size:11px}
.workflow-tabs{background:var(--ui-workspace)}
.workflow-canvas{background-color:var(--ui-canvas);background-image:radial-gradient(#d6dee1 1px,transparent 1px);background-size:20px 20px}
.canvas-intro{color:var(--ui-text-tertiary);font-size:11px}.canvas-intro strong{color:var(--ui-text-primary);font-size:14px}
.flow-column,.flow-node,.flow-empty{width:320px}
.flow-node{display:grid;min-height:112px;padding:0;grid-template-columns:1fr;gap:0;border-color:#dce4e6;border-left-width:4px;border-radius:8px;box-shadow:none;transition:border-color var(--ui-transition-fast),background-color var(--ui-transition-fast)}
.flow-node:hover{border-color:var(--ui-border-strong);transform:none}
.flow-node.selected{border-color:var(--ui-primary);box-shadow:var(--ui-focus)}
.flow-node.teal{border-left-color:var(--ui-primary)}.flow-node.blue{border-left-color:var(--ui-running)}.flow-node.amber{border-left-color:var(--ui-warning)}.flow-node.rose{border-left-color:var(--ui-danger)}
.node-type-header{display:flex;min-width:0;align-items:center;gap:8px;padding:8px 10px;border-bottom:1px solid var(--ui-border);color:var(--ui-text-tertiary);font:600 10px/1 var(--ui-font-mono);letter-spacing:.06em}.node-type-header>.node-icon{width:24px;height:24px}.node-type-header>.node-actions{margin-left:auto;flex-direction:row}.node-type-header>.node-actions :deep(.el-button){width:26px;height:26px;min-height:26px}
.node-icon{width:32px;height:32px;border-radius:6px;background:transparent;color:var(--ui-primary)}.node-icon.blue{background:transparent;color:var(--ui-running)}.node-icon.amber{background:transparent;color:var(--ui-warning)}.node-icon.rose{background:transparent;color:var(--ui-danger)}.node-icon.violet{background:transparent;color:#7669b5}
.node-copy{padding:11px 14px 13px}.node-copy>strong{font-size:14px;margin:0}.node-copy>small{color:var(--ui-text-secondary);font:11px/1.35 var(--ui-font-mono)}
.flow-link>span{border-left:1.5px solid #aab8bc}.flow-link>span::after{border-top-color:#aab8bc}.add-point{border-color:var(--ui-border-control);border-radius:6px;box-shadow:none;color:var(--ui-primary)}
.property-panel,.resource-panel{background:var(--ui-surface)}
.property-panel{border-color:var(--ui-border)}
.node-save-actions,.resource-actions{backdrop-filter:none;background:rgba(255,255,255,.98)}
@media(max-width:1250px){.flow-column,.flow-node,.flow-empty{width:300px}}
@media(max-width:767px){.flow-column,.flow-node,.flow-empty{width:min(320px,calc(100vw - 32px))}}
</style>

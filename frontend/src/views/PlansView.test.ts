...
describe('PlansView scenario list', () => {
  ...
  it('renders the required columns, Beijing times, workflow states, and operator actions', async () => {
    ...
    const wrapper = mount(PlansView, { attachTo: document.body, global: { plugins: [pinia, router, ElementPlus] } })
    await flushPromises()

    const headers = wrapper.findAll('.plan-list thead th').map(cell => cell.text())
    expect(headers).toEqual(['方案 / 场景名称', '创建时间', '更新时间', '工作流状态', '操作'])
    ...
    expect(wrapper.findAll('.scenario-row')).toHaveLength(3)
    const toggle = wrapper.get('[aria-label="收起方案 基础方案"]')
    await toggle.trigger('click')
    expect(wrapper.findAll('.scenario-row')).toHaveLength(0)
    await wrapper.get('[aria-label="展开方案 基础方案"]').trigger('click')
    expect(wrapper.findAll('.scenario-row')).toHaveLength(3)
    await wrapper.get('[aria-label="场景 已启用场景 的更多操作"]').trigger('click')
    await flushPromises()
    expect(document.body.textContent).toContain('基础信息')
    expect(document.body.textContent).toContain('复制场景')

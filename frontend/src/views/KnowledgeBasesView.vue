...
const loading = ref(false), saving = ref(false), error = ref(''), createVisible = ref(false), svnVisible = ref(false)
...
const visibleDocuments = computed(() => documents.value.filter(doc => (!origin.value || doc.origin === origin.value) && `${doc.name} ${doc.source_path}`.toLowerCase().includes(query.value.toLowerCase())))
...
watch(id, () => {
  current.value = null; documents.value = []; query.value = ''; origin.value = ''; searchQuery.value = ''; searchResults.value = []; searched.value = false; searchError.value = ''; searching.value = false; svnVisible.value = false
  void load(true)
  void loadModels().catch(cause => { error.value = errorMessage(cause) })
}, { immediate: true })
onBeforeUnmount(() => { ++loadSequence; clearTimeout(timer) })
</script>
    ...
    <el-tabs v-else-if="current" v-model="tab" class="detail-tabs">
      ...
      <el-tab-pane :label="`文档管理 (${current.document_count})`" name="documents">
        <section class="card section">
          <div class="document-toolbar"><div class="filters"><el-input v-model="query" clearable placeholder="搜索文件名称或路径" aria-label="搜索文档" :prefix-icon="Search" /><el-select v-model="origin" aria-label="文档来源" placeholder="全部来源"><el-option label="全部来源" value="" /><el-option label="SVN" value="svn" /><el-option label="手动上传" value="upload" /></el-select></div>
            <div class="actions"><el-button @click="svnVisible = !svnVisible">{{ svnVisible ? '收起 SVN 配置' : 'SVN 配置' }}</el-button><template v-if="auth.isAdmin"><input ref="uploadInput" class="file-input" type="file" multiple accept=".txt,.md,.csv,.json,.yaml,.yml,.htm,.html,.doc,.docx,.xls,.xlsx,.pdf" aria-label="上传文档" @change="upload" /><el-button :icon="Upload" :disabled="busy" :loading="uploading" @click="uploadInput?.click()">上传文件</el-button><el-button v-if="busy" :loading="saving" @click="index(true)">取消索引</el-button><el-button v-else type="primary" :icon="Refresh" :loading="saving" @click="index()">同步 / 重试索引</el-button></template></div>
          </div>
          <p class="muted">支持 PDF、Word、Excel、文本及结构化文件，单文件最大 50 MiB。SVN 文档通过同步范围管理，上传文件可单独删除。</p>
          <el-table v-if="visibleDocuments.length" :data="visibleDocuments" row-key="source_path"><el-table-column label="文档" min-width="240"><template #default="{ row }"><strong>{{ row.name }}</strong><small class="document-path">{{ row.source_path }}</small></template></el-table-column><el-table-column label="来源" width="110"><template #default="{ row }">{{ row.origin === 'svn' ? 'SVN' : '手动上传' }}</template></el-table-column><el-table-column prop="revision" label="版本" width="130" /><el-table-column label="大小" width="100"><template #default="{ row }">{{ (row.size / 1024).toFixed(1) }} KB</template></el-table-column><el-table-column prop="chunk_count" label="分块" width="80" /><el-table-column label="状态" min-width="130"><template #default="{ row }"><el-tag :type="row.status === 'failed' ? 'danger' : 'info'">{{ label(row.status) }}</el-tag><small v-if="row.error" class="document-path">{{ row.error }}</small></template></el-table-column><el-table-column v-if="auth.isAdmin" label="操作" width="90"><template #default="{ row }"><el-button v-if="row.upload_id" :icon="Delete" text type="danger" :disabled="busy || uploading" :aria-label="`删除 ${row.name}`" @click="remove(row)" /></template></el-table-column></el-table>
          <el-empty v-else :description="query || origin ? '没有匹配的文档' : '上传文件或配置 SVN 同步，开始构建知识库'" :image-size="80" />
        </section>
        <SvnSourcePanel v-if="svnVisible" :key="current.id" :knowledge-base-id="current.id" />
      </el-tab-pane>

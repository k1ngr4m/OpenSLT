...
<template>
  <div class="page users-page">
    ...
    <el-dialog v-model="dialog" :title="editing?'编辑用户':'新增用户'" width="520px"><el-form label-position="left" label-width="var(--ui-field-label-width)"><el-form-item label="用户名" required><el-input v-model="form.username" :disabled="!!editing" placeholder="用于登录，不可重复" /></el-form-item><el-form-item label="显示名称" required><el-input v-model="form.display_name" placeholder="界面中显示的姓名" /></el-form-item><el-form-item :label="editing?'重置密码':'初始密码'" :required="!editing"><el-input v-model="form.password" type="password" show-password :placeholder="editing?'留空表示不修改':'至少 8 个字符'" /><p class="field-help">{{ editing?'仅在需要重置时填写新密码':'用户首次登录后应立即修改初始密码' }}</p></el-form-item><el-form-item label="角色" required><el-select v-model="form.role" style="width:100%"><el-option v-for="(label,value) in roleText" :key="value" :label="label" :value="value"><div class="role-option"><strong>{{ label }}</strong><span>{{ roleDescription[value] }}</span></div></el-option></el-select><p class="field-help">{{ roleDescription[form.role] }}</p></el-form-item><el-form-item v-if="editing" label="账号状态"><el-switch v-model="form.is_active" active-text="启用" inactive-text="停用" /></el-form-item></el-form><template #footer><el-button @click="dialog=false">取消</el-button><el-button type="primary" :loading="saving" @click="save">保存用户</el-button></template></el-dialog>
  </div>
</template>

<style scoped>
.users-page{max-width:1600px}.user-summary{display:flex;overflow:hidden;width:max-content;margin-bottom:14px;border:1px solid var(--ui-border);border-radius:8px;background:var(--ui-surface)}.user-summary>div{min-width:130px;padding:12px 16px}.user-summary>div+div{border-left:1px solid var(--ui-border)}.user-summary span,.user-summary strong{display:block}.user-summary span{color:var(--ui-text-secondary);font-size:12px}.user-summary strong{margin-top:4px;font:650 20px/1.2 "Cascadia Code",Consolas,monospace}.keyword-filter{width:320px}.filter-count{margin-left:auto;color:var(--ui-text-secondary);font-size:12px}.table-panel{overflow:hidden}.user-cell{display:flex;align-items:center;gap:10px}.avatar{display:grid;flex:0 0 auto;width:32px;height:32px;place-items:center;border-radius:7px;color:var(--ui-primary-hover);background:var(--ui-primary-soft);font-size:12px;font-weight:700}.user-cell strong,.user-cell small{display:block}.user-cell strong{font-size:13px}.user-cell small{margin-top:2px;color:var(--ui-text-secondary);font-size:12px}.role-description,.table-time{color:var(--ui-text-secondary);font-size:12px}.field-help{margin:5px 0 0;color:var(--ui-text-secondary);font-size:12px;line-height:1.5}.role-option{display:grid;padding:4px 0}.role-option strong{font-size:12px}.role-option span{color:var(--ui-text-secondary);font-size:12px}@media(max-width:767px){.user-summary{width:100%}.user-summary>div{min-width:0;flex:1}.keyword-filter{width:100%}}
</style>
